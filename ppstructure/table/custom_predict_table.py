...
class TableSystem(object):
    def __init__(self, args, text_detector=None, text_recognizer=None):
        ...
        self.text_recognizer = get_ocr_predictor(
                    model_dir="/Users/tieuanhnguyen/PycharmProjects/VND_VAT/vnd-pti-ocr/models/pti_ocr", model_type="seq2seq",
                    model_name="vgg_seq2seq_vat_2510_20231026", device="cpu", num_torch_threads=2
                )
        # vnd_log.dlog_i('OCR predictor is loaded')
        if benchmark_tmp:
            args.benchmark = True
    ...
    def _ocr(self, img):
        ...
        rec_res = text_recog(self.text_recognizer, img_crop_list, prob_threshold=0.5)
        rec_elapse = 23
...
def text_recog(text_recognizer, img_list, prob_threshold, avg_prob=0.55):
    results = []
    for index, item in enumerate(img_list):
        # if abs(item.top - item.bot) > abs(item.left - item.right):
        #     continue
        img = im.fromarray(item)
        label, prob = text_recognizer.predict(img)
